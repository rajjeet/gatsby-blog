export default {
  primaryColor: '#2185d0',
  secondaryColor: '#f3711c',
};

export const computerBreakpoint = '1024px';
export const tabletBreakpoint = '769px';
export const bigMobileBreakpoint = '426px';
export const smallMobileBreakpoint = '321px';
export const breakpoint600 = '600px';

export const boxShadow = '0 4px 8px 0 rgba(0,0,0,0.2)';
export const hoverBoxShadow = '0 4px 9px 0 rgba(0,0,0,0.6)';
export const lightBoxShadow = '0 4px 4px 0 rgba(0,0,0,0.15)';

export const primaryColor = '#2185d0';
export const secondaryColor = '#186199';
export const primaryTextColor = '#333';

export const borderRadius = '5px';

export const smallFontSize = '.8em';
